...
cust_addre_cte as (
	...
	from 
		{{ ref('stg_customer') }} as cus
	left join 
		{{ ref('stg_address') }} as add on cus.address_id = add.address_id
...
store_manager_cte as (
	...
	from
		{{ ref('stg_store') }} as sto
	left join
		{{ ref('stg_staff') }} as stf on sto.manager_staff_id = stf.staff_id
